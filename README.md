...
Recurrent neural networks (RNN) can implement real-time virtual-analog audio effects. Such models use error-to-signal ratio (ESR) for evaluation, followed by perceptual evaluation. Masking-based objective metrics, Noise-to-Mask Ratio (NMR) and Perceptual Evaluation of Audio Quality (PEAQ), can serve as a word-length selection criterion alongside ESR. We deploy a single gated recurrent unit (GRU) kernel on an xc7z020 FPGA with six weight sets (three guitar pedals, two gains) and sweep the fixed-point word-length (W) from W = 8 to 24 and score each output against its float32 reference under ESR, NMR and PEAQ. At literature-motivated operating points (ESR <= -20 dB, PEAQ objective difference grade (ODG) >= -0.5), ESR selects word-lengths a mean of 4.2 bits narrower than PEAQ over six models, five trainval sources. Also, NMR worsens for four models at model-specific word-lengths within W = 11 to 14 while ESR improves monotonically, coinciding with transitions in coherence and spectral flatness.
...
* **Single kernel.** One HLS source and architecture (`hls/src/gru_va_col_fused_mac3.cpp`) compiled separately for each of the 102 (weight set, W) builds with the weights as constants — not one runtime-switchable bitstream.
