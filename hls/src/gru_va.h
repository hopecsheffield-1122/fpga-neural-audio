...
void gru_va(hls::stream<axis_sample> &audio_in,
            ...
            const float *weights,   // m_axi: flat FP32 buffer; only read in the alternate runtime-load build (unused under USE_COMPILED_WEIGHTS, the reported configuration)
            const int mode,         // 0 = preload weights (no-op under USE_COMPILED_WEIGHTS), 1 = run inference
